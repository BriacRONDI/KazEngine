 ...
 * Constructeur
 */
FbxParser::FbxParser(std::vector<char> const& data) : data(data)
{
    #if defined(_WIN32)
 ...
 */
void FbxParser::SetTextColor(uint8_t color)
{
    #if defined(_WIN32)
...
std::vector<char> FbxParser::ParseData(std::string const& texture_directory)
{
    // On vérifie l'emprunte FBX
    std::cout << "Checking FBX tag : ";

    if(!this->isBinaryFbx()) {
        this->SetTextColor(TEXT_COLOR_RED);
        std::cout << "FAILED" << std::endl;
        this->SetTextColor();  
        return {};
    }

    this->SetTextColor(TEXT_COLOR_GREEN);
    std::cout << "OK" << std::endl;
    this->SetTextColor();

    std::cout << "FBX Version : ";
    this->SetTextColor(TEXT_COLOR_BLUE);
    ...
    Engine::Packer::PackToMemory(memory, "/", Engine::Packer::DATA_TYPE::PARENT_NODE, "textures", {}, 0);
    Engine::Packer::PackToMemory(memory, "/", Engine::Packer::DATA_TYPE::PARENT_NODE, "materials", {}, 0);
    Engine::Packer::PackToMemory(memory, "/", Engine::Packer::DATA_TYPE::PARENT_NODE, "meshes", {}, 0);

    if(!this->bone_trees.empty())
        Engine::Packer::PackToMemory(memory, "/", Engine::Packer::DATA_TYPE::PARENT_NODE, "bones", {}, 0);
    ...
    for(auto const& tree : this->bone_trees) {
        ...
        Engine::Packer::PackToMemory(memory, "/bones", Engine::Packer::DATA_TYPE::BINARY_DATA, tree.first, serialized_tree, static_cast<uint32_t>(serialized.size()));
    ...
    for(auto& mesh : this->meshes) {
        ...
        for(auto& material : mesh.materials) {
            ...
            && this->engine_materials.count(material.first) > 0) {
                ...
                Engine::Packer::PackToMemory(memory, "/materials", Engine::Packer::DATA_TYPE::BINARY_DATA, material.first, serialized_material, serialized_material_size);
                ...
                && std::find(added_textures.begin(), added_textures.end(), this->engine_materials[material.first].texture) == added_textures.end()) {
                    ...
                    for(auto& texture : this->textures) {
                        if(Engine::Tools::GetFileName(texture.RelativeFilename) == filename) {
                            file_content = Engine::Tools::GetBinaryFileContents(texture_directory + '/' + texture.RelativeFilename);
                            break;
                        }
                    }
                    if(file_content.empty()) {
                        ...
                        Engine::Packer::PackToMemory(memory, "/textures", Engine::Packer::DATA_TYPE::BINARY_DATA,
        ...
        Engine::Packer::PackToMemory(memory, "/meshes", Engine::Packer::DATA_TYPE::BINARY_DATA, mesh.name, serialized_mesh, serialized_mesh_size);
...
Engine::BONE FbxParser::CreateBone(FBX_NODE const& node, Engine::BONE const& parent)
{
    ...
    Engine::Matrix4x4 parent_transformation = IDENTITY_MATRIX;
...
void FbxParser::ParseAnimationStack(std::vector<FBX_NODE> const& nodes)
{
    ...
    for(auto const& node : animation_stack) {
        ...
        if(properties70.count("LocalStop") > 0) animation_time = fbx_duration(Engine::Packer::ParseInt64(this->data, properties70["LocalStop"].value[0].position + 1));
                                    ...
                                    for(auto& model : models) {
                                        if(model.id == parent.id) {
                                            ...
                                            if(animation_layer.animation_nodes.count(model.attribute_name) > 0) {
                                                ...
                                                // if(this->referenced_bones.count(model.id) > 0)
                                                //    anim_node.transform = this->FindCluster(model.id).transform_link;
    ...
    for(auto const& node : anim_curve_nodes) {
        ...
        for(auto& key : search) {
            if(node.attribute_name == key) {
                ...
                if(properties70.count("d|X") > 0) curve_node.vector[0] = (float)Engine::Packer::ParseFloat64(this->data, properties70["d|X"].value[0].position + 1);
                if(properties70.count("d|Y") > 0) curve_node.vector[1] = (float)Engine::Packer::ParseFloat64(this->data, properties70["d|Y"].value[0].position + 1);
                if(properties70.count("d|Z") > 0) curve_node.vector[2] = (float)Engine::Packer::ParseFloat64(this->data, properties70["d|Z"].value[0].position + 1);
                if(properties70.count("d|DeformPercent") > 0) curve_node.morph = Engine::Packer::ParseFloat64(this->data, properties70["d|DeformPercent"].value[0].position + 1);
            ...
            for(auto const& bone_node : deformer_nodes) {
                if(bone_node.id == child.id) {
                    ...
                    if(bone_node.children.count("Mode") > 0)
                        bone.link_mode = Engine::Packer::ParseInt32(this->data, FBX_PROPERTY_DATA(bone_node.children,"Mode").position);
...
void FbxParser::ParseMeshes(std::vector<FBX_NODE> const& nodes)
{
    ...
    for(auto mesh : model_nodes) {
        ...
        /*if(mesh.children.count("Culling") > 0) {
            ...
            if(culling == "CullingOff") serialized_mesh.culling = Engine::Mesh::BACKFACE_CULLING::CULLING_NONE;
            else if(culling == "CullingOnCCW") serialized_mesh.culling = Engine::Mesh::BACKFACE_CULLING::CULLING_CLOCKWISE;
            else if(culling == "CullingOnCW") serialized_mesh.culling = Engine::Mesh::BACKFACE_CULLING::CULLING_COUNTER_CLOCKWISE;
        }*/
            ...
            if(signed_vertex_index < 0) {
                ...
                if(!mesh_geometry.material.indices.empty() && mesh_geometry.material.mapping_information_type == "ByPolygon") {
                    ...
                    if(fbx_face_index > mesh_geometry.material.indices.size()) {
                        ...
                        this->SetTextColor(TEXT_COLOR_ORANGE);
...
std::vector<FbxParser::FBX_LAYER> FbxParser::ParseLayers(FBX_NODE const& layer_node)
{
    ...
    for(auto& element : layer_node.children.at("LayerElement")) {
        ...
        layer.TypedIndex = Engine::Packer::ParseUint32(this->data, FBX_PROPERTY_DATA(element.children,"TypedIndex").position);
...
void FbxParser::ParseMaterials(std::vector<FBX_NODE> const& nodes)
{
    ...
    for(auto node : material_nodes) {
        ...
        if(properties70.count("EmissiveFactor") > 0) material.EmissiveFactor = Engine::Packer::ParseFloat64(this->data, properties70["EmissiveFactor"].value[0].position + 1);
        if(properties70.count("AmbientFactor") > 0) material.AmbientFactor = Engine::Packer::ParseFloat64(this->data, properties70["AmbientFactor"].value[0].position + 1);
        if(properties70.count("DiffuseFactor") > 0) material.DiffuseFactor = Engine::Packer::ParseFloat64(this->data, properties70["DiffuseFactor"].value[0].position + 1);
        if(properties70.count("SpecularFactor") > 0) material.SpecularFactor = Engine::Packer::ParseFloat64(this->data, properties70["SpecularFactor"].value[0].position + 1);
        if(properties70.count("BumpFactor") > 0) material.BumpFactor = Engine::Packer::ParseFloat64(this->data, properties70["BumpFactor"].value[0].position + 1);
        if(properties70.count("TransparencyFactor") > 0) material.TransparencyFactor = Engine::Packer::ParseFloat64(this->data, properties70["TransparencyFactor"].value[0].position + 1);
        if(properties70.count("Shininess") > 0) material.Shininess = Engine::Packer::ParseFloat64(this->data, properties70["Shininess"].value[0].position + 1);
        ...
        Engine::Mesh::MATERIAL engine_material;
        ...
        if(material.textures.count("DiffuseColor") > 0
            ...
            engine_material.texture = Engine::Tools::GetFileName(material.textures["DiffuseColor"]->RelativeFilename);
...
std::array<double, 3> FbxParser::ParseColor(FBX_PROPERTY70 const& property70)
{
    ...
    color[0] = Engine::Packer::ParseFloat64(this->data, property70.value[0].position + 1);
    color[1] = Engine::Packer::ParseFloat64(this->data, property70.value[1].position + 1);
    color[2] = Engine::Packer::ParseFloat64(this->data, property70.value[2].position + 1);
...
Engine::Matrix4x4 FbxParser::ComputeTransformation(FBX_NODE const& node, std::vector<FBX_NODE> const& models)
{
...
Engine::Matrix4x4 FbxParser::ComputeTransformation(FBX_NODE const& node, Engine::Matrix4x4 const& parent_transformation)
{
    Engine::Matrix4x4::EULER_ORDER euler_order = Engine::Matrix4x4::EULER_ORDER::ZYX;
    Engine::Matrix4x4 lTranslationM;
	Engine::Matrix4x4 lPreRotationM;
	Engine::Matrix4x4 lRotationM;
	Engine::Matrix4x4 lPostRotationM;

	Engine::Matrix4x4 lScalingM;
	Engine::Matrix4x4 lScalingPivotM;
	Engine::Matrix4x4 lScalingOffsetM;
	Engine::Matrix4x4 lRotationOffsetM;
	Engine::Matrix4x4 lRotationPivotM;
	
	Engine::Matrix4x4 lGlobalT;
    ...
    if(properties70.count("RotationOrder") > 0) {
        std::string rotation_order = Engine::Packer::ParseString(
            this->data,
        ...
        if(rotation_order == "ZYX") euler_order = Engine::Matrix4x4::EULER_ORDER::ZYX;
        if(rotation_order == "YZX") euler_order = Engine::Matrix4x4::EULER_ORDER::YZX;
        if(rotation_order == "XZY") euler_order = Engine::Matrix4x4::EULER_ORDER::XZY;
        if(rotation_order == "ZXY") euler_order = Engine::Matrix4x4::EULER_ORDER::ZXY;
        if(rotation_order == "YXZ") euler_order = Engine::Matrix4x4::EULER_ORDER::YXZ;
        if(rotation_order == "XYZ") euler_order = Engine::Matrix4x4::EULER_ORDER::XYZ;
    }

    if(properties70.count("Lcl_Translation") > 0) lTranslationM = Engine::Matrix4x4::TranslationMatrix(Engine::Vector3(this->ParseVector(properties70["Lcl_Translation"])));

    if(properties70.count("PreRotation") > 0) {
        Engine::Vector3 vector(this->ParseVector(properties70["PreRotation"]));
        lPreRotationM = Engine::Matrix4x4::EulerRotation(lPreRotationM, vector.ToRadians(), euler_order);
    }

    if(properties70.count("Lcl_Rotation") > 0) {
        Engine::Vector3 vector(this->ParseVector(properties70["Lcl_Rotation"]));
        lPreRotationM = Engine::Matrix4x4::EulerRotation(lPreRotationM, vector.ToRadians(), euler_order);
    }

    if(properties70.count("PostRotation") > 0) {
        Engine::Vector3 vector(this->ParseVector(properties70["PostRotation"]));
        lPreRotationM = Engine::Matrix4x4::EulerRotation(lPreRotationM, vector.ToRadians(), euler_order);
    }

    if(properties70.count("Lcl_Scaling") > 0) {
        Engine::Vector3 scale(this->ParseVector(properties70["Lcl_Scaling"]));
        lScalingM = Engine::Matrix4x4::ScalingMatrix(scale);
    }

    // Pivots and offsets
    if(properties70.count("ScalingOffset") > 0) lScalingOffsetM = Engine::Matrix4x4::TranslationMatrix(Engine::Vector3(this->ParseVector(properties70["ScalingOffset"])));
    if(properties70.count("ScalingPivot") > 0) lScalingPivotM = Engine::Matrix4x4::TranslationMatrix(Engine::Vector3(this->ParseVector(properties70["ScalingPivot"])));
    if(properties70.count("RotationOffset") > 0) lRotationOffsetM = Engine::Matrix4x4::TranslationMatrix(Engine::Vector3(this->ParseVector(properties70["RotationOffset"])));
    if(properties70.count("RotationPivot") > 0) lRotationPivotM = Engine::Matrix4x4::TranslationMatrix(Engine::Vector3(this->ParseVector(properties70["RotationPivot"])));

    // parent transform
	Engine::Matrix4x4 lParentGX(parent_transformation);

    // Global Rotation
	Engine::Matrix4x4 lLRM = lPreRotationM * lRotationM * lPostRotationM;
	Engine::Matrix4x4 lParentGRM = lParentGX.ExtractRotation();

    // Global Shear * Scaling
	Engine::Matrix4x4 lParentTM;
	Engine::Matrix4x4 lLSM = lScalingM;
	Engine::Matrix4x4 lParentGSM;
	Engine::Matrix4x4 lParentGRSM;
    ...
    Engine::Matrix4x4 lGlobalRS;

    uint32_t inherit_type = 0;
    if(properties70.count("InheritType") > 0)
        inherit_type = Engine::Packer::ParseUint32(this->data, properties70["InheritType"].value[0].position + 1);
	...
	if(inherit_type == 0) {
		...
		Engine::Matrix4x4 lParentGSM_noLocal = lParentGSM * lScalingM.Inverse();
		lGlobalRS = lParentGRM * lLRM * lParentGSM_noLocal * lLSM;
	}

    // Calculate the local transform matrix
	Engine::Matrix4x4 lTransform = lTranslationM * lRotationOffsetM * lRotationPivotM * lPreRotationM * lRotationM * lPostRotationM * lRotationPivotM.Inverse() * lScalingOffsetM * lScalingPivotM * lScalingM * lScalingPivotM.Inverse();

	Engine::Matrix4x4 lLocalTWithAllPivotAndOffsetInfo = lTransform.ToTranslationMatrix();

	Engine::Matrix4x4 lGlobalTranslation = lParentGX * lLocalTWithAllPivotAndOffsetInfo;
...
void FbxParser::ParseConnections(std::vector<FBX_NODE> const& nodes)
{
    ...
    for(auto& node : connections_node.children["C"]) {
        ...
        parent.id = Engine::Packer::ParseInt64(this->data, node.properties[1].position + 1);
        child.id = Engine::Packer::ParseInt64(this->data, node.properties[2].position + 1);
...
{
    return Engine::Packer::ParseUint32(this->data, 23);
}
...
FbxParser::FBX_NODE FbxParser::ParseNode(size_t& position)
{
    ...
    if(this->version > 7500) {
        ...
        end_offset = Engine::Packer::ParseUint64(this->data, position);
        position += sizeof(uint64_t);

        num_properties = Engine::Packer::ParseUint64(this->data, position);
        position += sizeof(uint64_t);

        property_list_length = Engine::Packer::ParseUint64(this->data, position);
        position += sizeof(uint64_t);
    }else{

        end_offset = static_cast<uint64_t>(Engine::Packer::ParseUint32(this->data, position));
        position += sizeof(uint32_t);

        num_properties = static_cast<uint64_t>(Engine::Packer::ParseUint32(this->data, position));
        position += sizeof(uint32_t);

        property_list_length = static_cast<uint64_t>(Engine::Packer::ParseUint32(this->data, position));
        position += sizeof(uint32_t);
    }
    ...
    node.name = Engine::Packer::ParseString(this->data, static_cast<uint32_t>(position), name_length);
    ...
    if(node.properties.size() > 0) {
        switch(node.properties[0].type) {
            case 'Y' : node.id = static_cast<int64_t>(Engine::Packer::ParseInt16(this->data, node.properties[0].position + 1)); break;
            case 'I' : node.id = static_cast<int64_t>(Engine::Packer::ParseInt32(this->data, node.properties[0].position + 1)); break;
            case 'L' : node.id = Engine::Packer::ParseInt64(this->data, node.properties[0].position + 1); break;
...
FbxParser::FBX_PROPERTY FbxParser::ParseRawProperty(size_t position)
{
    ...
    switch(type) {
        ...
        case 'R' : 
        case 'S' : return {type, position, static_cast<size_t>(Engine::Packer::ParseUint32(this->data, position + offset) + sizeof(uint32_t) + offset)};
        ...
        case 'l' :
        {
            uint32_t array_length = Engine::Packer::ParseUint32(this->data, position + offset);
            offset += sizeof(uint32_t);

            uint32_t encoding = Engine::Packer::ParseUint32(this->data, position + offset);
            offset += sizeof(uint32_t);

            uint32_t compressed_length = Engine::Packer::ParseUint32(this->data, position + offset);
            offset += sizeof(uint32_t);
...
std::vector<double> FbxParser::ParseFloat64Array(FBX_PROPERTY const& property)
{
    ...
    uint32_t array_length = Engine::Packer::ParseUint32(this->data, property.position + offset);
    offset += sizeof(uint32_t);

    uint32_t encoding = Engine::Packer::ParseUint32(this->data, property.position + offset);
    offset += sizeof(uint32_t);

    uint32_t compressed_length = Engine::Packer::ParseUint32(this->data, property.position + offset);
    offset += sizeof(uint32_t);
    ...
    for(uint32_t i=0; i<array_length; i++) {
        ...
        double value;
        if(Engine::Tools::IsBigEndian()) {
            #if defined(_WIN32)
            uint64_t uivalue = *reinterpret_cast<const uint64_t*>(data.data() + i * sizeof(uint64_t));
            uivalue =_byteswap_uint64(uivalue);
            value = *reinterpret_cast<const double*>(&uivalue);
            #endif
        }else{
            value = *reinterpret_cast<const double*>(data.data() + i * sizeof(double));
        }
...
std::vector<int32_t> FbxParser::ParseInt32Array(FBX_PROPERTY const& property)
{
    ...
    for(uint32_t i=0; i<array_length; i++) {
        ...
        if(Engine::Tools::IsBigEndian()) value = _byteswap_ulong(value);
        output.push_back(value);
    }
...
/**
 * Alias local de la fonction ParseString de Engine::Packer
 */
std::string FbxParser::ParseString(FBX_PROPERTY const& property)
{
    return Engine::Packer::ParseString(this->data, static_cast<uint32_t>(property.position+sizeof(uint32_t)+1), static_cast<uint32_t>(property.size-sizeof(uint32_t)-1));
}