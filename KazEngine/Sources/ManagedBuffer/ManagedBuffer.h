...
namespace Engine
{
    class ManagedBuffer
    {
        public :
            ...
            void ReadData(std::shared_ptr<Chunk> chunk, uint8_t instance_id);
            bool FlushWrite(Vulkan::COMMAND_BUFFER const& command_buffer);
            bool FlushWrite(Vulkan::COMMAND_BUFFER const& command_buffer, uint8_t instance_id, VkSemaphore signal = nullptr, VkSemaphore wait = nullptr);
            bool FlushRead(Vulkan::COMMAND_BUFFER const& command_buffer, uint8_t instance_id, VkSemaphore signal = nullptr, VkSemaphore wait = nullptr);
            ...
            inline void ReadStagingBuffer(void* data, VkDeviceSize size, VkDeviceSize offset, uint8_t instance_id) { std::memcpy(data, this->staging_buffers[instance_id].pointer + offset, size); }
        ...
        private :
            ...
            std::vector<std::vector<Chunk>> read_chunks;
