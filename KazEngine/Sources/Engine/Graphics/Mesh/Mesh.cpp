...
namespace Engine
{
     ...
     */
    std::unique_ptr<char> Mesh::BuildSubMeshVBO(VkDeviceSize& output_size)
    {
        //////////////////////////
        // Calcul de la taille  //
        // du buffer de sortie  //
        ...
        if(!this->uv_index.empty()) {
            ...
            for(uint32_t i=0; i<this->materials.size(); i++) {
                ...
                for(auto& face_index : material.second) {
                    ...
                    for(uint8_t j=0; j<3; j++) {
                        ...
                        std::memcpy(output.get() + offset, &this->vertex_buffer[vertex_index], sizeof(Vector3));
                        ...
                        std::memcpy(output.get() + offset, &this->uv_buffer[uv_index], sizeof(Vector2));
            ...
            for(uint32_t i=0; i<this->vertex_buffer.size(); i++) {
                ...
                // Position
                std::memcpy(output.get() + offset, &this->vertex_buffer[i], sizeof(Vector3));
                offset += sizeof(Vector3);
                ...
                if(!this->uv_buffer.empty()) {
                    std::memcpy(output.get() + offset, &this->uv_buffer[i], sizeof(Vector2));
                    offset += sizeof(Vector2);
                }
            }
     ...
     */
    std::unique_ptr<char> Mesh::BuildVBO(VkDeviceSize& output_size)
    {
    ...
    {
        output_size = static_cast<uint32_t>(sizeof(Vector4) * 3 + sizeof(float) + sizeof(uint8_t) + this->texture.size());
        std::unique_ptr<char> output(new char[output_size]);
        uint32_t offset = 0;
        *reinterpret_cast<Vector4*>(output.get() + offset) = this->ambient;
        offset += sizeof(Vector4);
        *reinterpret_cast<Vector4*>(output.get() + offset) = this->diffuse;
        offset += sizeof(Vector4);
        *reinterpret_cast<Vector4*>(output.get() + offset) = this->specular;
        offset += sizeof(Vector4);
    ...
    {
        uint32_t offset = 0;
        this->ambient = *reinterpret_cast<const Vector4*>(data + offset);
        offset += sizeof(Vector4);
        this->diffuse = *reinterpret_cast<const Vector4*>(data + offset);
        offset += sizeof(Vector4);
        this->specular = *reinterpret_cast<const Vector4*>(data + offset);
        offset += sizeof(Vector4);
