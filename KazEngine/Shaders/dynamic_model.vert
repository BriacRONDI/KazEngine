...
layout (set=3, binding=0) buffer Skeleton
{
	mat4 bones[];
} skeleton;

layout (set=3, binding=1) buffer OffsetIDs
{
	uint offset_ids[];
};

layout (set=3, binding=2) buffer Offsets
{
	mat4 offsets[];
};

layout (set=3, binding=3) buffer Animations
{
	uint bone_count;
	uint first_frame_id[];
}animations;
